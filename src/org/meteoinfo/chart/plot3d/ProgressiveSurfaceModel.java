...
/**
 * {@link ProgressiveSurfaceModel} fills the surface with increasing
 * "definition".
 * <p>
 * It allocate an HD array , and start with a single face, then 4, then 16 etc.
 * until reaching the goal HD definition.
 * <p>
 * This defines the
 *
 * <pre>
...
public class ProgressiveSurfaceModel extends AbstractSurfaceModel implements SurfaceModel {
    ...
    private SurfaceVertex[][] extractResolution(int def) {
        ...
        for (int i = 0; i < k; i++) {
            for (int j = 0; j < k; j++) // i, and j are in def coordinate
            {
                copy(i, j, def, vertex);
            }
        }
    ...
    /**
     * copy surfaceVertex from the highdefinition in coordinate i,j, in
     * definition def, into the target
     *
     * @param i
     * @param j
     * @param def
    ...
    /**
     * called only when data are available in the given definition, this causes
     * a lot of changes
     *
