...
//	}
    ...
    private void plotSurface() {
        ...
        if ((cop.x > 10) || (cop.x < -10)) {
            if ((cop.y > 10) || (cop.y < -10)) {
                ...
                plotArea(start_lx, 0, end_lx, split_y, sx, ymultiple_factor);
                plotArea(start_lx, ycalc_divisions, end_lx, split_y, sx, -ymultiple_factor);
            }
            ...
            int split_y = (int) ((cop.y + 10) * yplot_density / 20) * ymultiple_factor;
            critical = true;
            plotArea(0, 0, split_x, split_y, multiple_factor, ymultiple_factor);
            plotArea(0, ycalc_divisions, split_x, split_y, multiple_factor, -ymultiple_factor);
            plotArea(calc_divisions, 0, split_x, split_y, -multiple_factor, ymultiple_factor);
            plotArea(calc_divisions, ycalc_divisions, split_x, split_y, -multiple_factor, -ymultiple_factor);
        }
        ...
        for (int counter = 0; counter <= contour_lines + 1; counter++) {
            ...
            for (int edge = 0; edge < 4; edge++) {
                ...
                if (xpoints[index] != -2) {
                    ...
                    } else if ((z > contour_vertex[edge].z) || (z > contour_vertex[nextedge].z)) {
                        switch (index) {
                            case 1:
    ...
    private void plotContour() {
        ...
        if (plotfunc1 || plotfunc2) {
            ...
            int ydelta = (ycalc_divisions + 1) * ymultiple_factor;
            for (int i = 0; i < ycalc_divisions; i += ymultiple_factor) {
    ...
    private void plotWireframe() {
        ...
        for (int func = 0; func < 2; func++) {
            ...
            while (j <= calc_divisions) {
                ...
                i = 0;
                k = ++j;
                counter = (counter + 1) % ymultiple_factor;
            }
        }
        if (isBoxed) {
            drawBoundingBox();
